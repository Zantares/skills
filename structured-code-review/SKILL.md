...
- **输出检视意见**：问题列表含**可跳转源码位置**（path + 行号，及/或短代码引用块）、严重性、问题描述、**修复建议**（说明作者应如何改，可附示例片段，但标明「建议」）。无行号的条目视为未完成。
...
**源码可追溯（强制）**：每条问题必须带可跳转的源码定位（行号引用和/或短代码片段），见 [源码可追溯要求](#源码可追溯要求强制)。禁止只写函数名/阶段名而无行号。
...
- [ ] 已执行 Common Checks
- [ ] 问题含全局序号、位置（**path + 行号**）、严重性、修复建议
- [ ] 每条问题含代码引用块或等价可跳转行号（见源码可追溯要求）
...
生成问题列表时**必须**同时满足：**按优先级分组** + **每条全局序号** + **源码可追溯**。
...
每条检视问题必须让读者**不靠猜**就能打开对应代码。输出时遵守：

1. **位置字段必填**：写清 `filepath` + **起止行号**（或单行）。优先使用 Cursor 可跳转引用：
   - 行内：`` `startLine:endLine:path` ``
   - 或代码引用块：
     ````markdown
     ...
     ````
2. **片段或引用二选一（推荐两者都有）**：
   - **短片段**（通常 3–15 行）：直接展示有问题的语句/条件；或
   - **仅行号引用**：当片段过长、或同文件多处重复同一模式时，可用一行位置 + 一句「见该处 `xxx`」。
3. **多位置问题**：主因一处写完整引用；关联处用额外子弹列出（如「检测路径 `A:L1-L2` vs 改写路径 `B:L3-L4`」）。
4. **禁止**：只有「在 `GetDynamicModule` / 阶段 5 / reshape 分支」这类描述、**无行号**；或只贴伪代码不指向仓库真实行。
5. **Commit 模式**：行号以当前工作区可读文件为准；若仅存在于某 commit，注明 `commit:<hash>` 并尽量 `git show hash:path` 对照，或引用 diff hunk 中的 `@@` 行。

自检：把问题列表发给未参加对话的人，能否凭 **#n → 打开文件行号** 完成追溯？不能则补引用。
...
### 严重
#### #1 \<简短标题\>
- **位置**：`1808:1912:xla/service/epu/offline_execution.cc`（reshape 挂接）
- **源码**：
  ```1808:1912:xla/service/epu/offline_execution.cc
  // 出问题的关键行（可截断）
  ```
- **问题**：…
- **修复建议**：…
...
也可用表格，但须保留 **`#n` 列**，且 **位置列必须含 path:lines**（不可只写符号名）：

| # | 严重性 | 位置 | 问题 | 修复建议 |
|---|--------|------|------|----------|
| 1 | 严重 | `file.cc:1907-1912` | … | … |

长片段不必塞进表格；表格内写短位置，详情仍用上方 `#### #n` 块 + 代码引用。

### 无问题 / 仅观察

- 若无严重/中等/低项：写「未发现问题」，可省略序号。
- 非缺陷的观察（如测试建议）放在 **其他结论**，不占用 `#n`，除非用户明确要求纳入问题列表；观察若指向具体代码，也建议带行号。
...
- [ ] 已给出可运行的验证建议（若仓库可访问）
- [ ] 问题列表含全局连续序号 `#n`
- [ ] 每条问题含 **path + 行号** 及代码引用/短片段（见源码可追溯要求）
- [ ] 未擅自修改源码（除非用户明确要求实现修复）
...
**Mode A 在流程.walk 之后执行；Mode B 在逐文件说明之后执行。** 下列三类须逐项过一遍，并给出**具体位置**（必须可追溯到文件行号；见 [源码可追溯要求](#源码可追溯要求强制)）。记入 [问题列表格式](#问题列表格式) 时，每条分配一个全局 `#n`。
...
- 代码引用：问题列表与流程说明中的关键逻辑，统一用 `` `startLine:endLine:path` `` 或 fenced ` ```startLine:endLine:path ` ` 块，便于跳转。
- Maintainer 默认**只读**仓库；检视结论通过对话或 `*_REVIEW.md` 交付，不改动作者分支上的实现文件。
- 交付前快速扫一遍问题列表：无行号的条目一律补全后再发出。
