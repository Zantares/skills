...
- **功能总结**（A.1）
- **流程检视**（A.2，可分阶段小标题）
- **问题列表**（Common Checks，含位置/问题/严重性/修复建议）
...
2. **修改目的与思路**
3. **逐文件改动说明**（含核心点与难点）
4. **设计缺陷与风险**（Common Checks 结果，按严重/中等/低分组）
...
**Mode A 在流程.walk 之后执行；Mode B 在逐文件说明之后执行。** 下列三类须逐项过一遍，并给出**具体位置**（commit 模式引用文件+函数/行；pass 模式引用阶段或行号）。
